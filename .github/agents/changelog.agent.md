...
- Upload de installer asset met versienaam `VBEAddIn-Installer-X.Y.Z.exe`
- Houd minimaal de huidige + ~8 oudere releases installeerbaar

Voorbeeld met GitHub CLI:
```powershell
gh release create X.Y.Z "Installer/bin/Release/VBEAddIn-Installer-X.Y.Z.exe" --repo jeroenfledderus1991-droid/VBAUtilities --title "X.Y.Z" --notes "Release X.Y.Z"
```

Controleer daarna dat deze URL werkt (HTTP 302/200):
```text
https://github.com/jeroenfledderus1991-droid/VBAUtilities/releases/download/X.Y.Z/VBEAddIn-Installer-X.Y.Z.exe
```
...
- Bij elke nieuwe versie moet ook een GitHub Release met `VBEAddIn-Installer-X.Y.Z.exe` worden gepubliceerd
